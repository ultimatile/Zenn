...
ユーザー名前空間が有効で、`ignored-acls`でLustreをなだめ、flakeを避けることで、共同利用スパコンの永続ホーム上で素のNixが動いてNeovimが入りました。 逆にユーザー名前空間が無効なスパコンではnix-user-chrootが使えないため、nix-portable（proot）やコンテナを検討することになります。
