...
1の「外堀を書いていないだけ」をもう少し掘り下げると、実務で本当に見逃されるのは**暗黙仕様**です。アルゴリズムの数学的契約（SVDの再構成精度、直交性等）は仕様から自明であり、テストを書くのは難しくありません。一方で、実装レベルの暗黙仕様——例えばメモリレイアウトが結果に影響しない、演算順序が結果に影響しない、内部表現の選択が外部挙動に影響しない——はどこにも明文化されておらず、テストからも漏れがちです。

実際にテンソル計算ライブラリの開発で観察されたバグの多くは、この暗黙仕様の違反でした。column-majorレイアウトで次元が逆になる、einsumの出力インデックス順序が無視される、strideフィールドとorderフィールドの不整合でsilent data corruptionが起きる——いずれもアルゴリズムの数学的正しさとは別の層の問題です。

この種の暗黙仕様を体系的に発見する方法は、残念ながら知られていません。Daikonのようなinvariant minerは実行トレースから表層的な関係を検出しますが、「メモリレイアウトを変えても結果が同じ」のような性質はトレースからは見えません。LLMに仮説生成させるアプローチも検討しましたが、対象コードベース固有のバグパターンを推測するには限界があります。

現実的に機能するのは**事後的なアプローチ**です。バグが出たとき（あるいはレビューで指摘されたとき）、その修正で終わらせず「このバグはどの暗黙仕様の違反か」と問い、その仕様を一般的な契約テストに昇格させる。1つのfixは1つのバグを防ぎますが、1つの契約テストはそのクラス全体を防ぎます。地味ですが、テストカバレッジは結局この積み重ねでしか育ちません。
