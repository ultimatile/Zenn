...
スパコンでの開発は、手元でコードを書いて、rsyncで送って、sshしてsbatch叩いて、結果をcatして...というサイクルの繰り返しです。
rsyncのオプション、ジョブスクリプトの記述、module loadの手順など、毎回ちゃんと書くのはめんどくさいです。
...
やることはシンプルで、**コーディングエージェントはローカルで動かし、スパコンには計算だけを投げる**だけです。
...
ファイルの探索やコード編集はローカルファイルシステムで行い、rsyncで差分同期してからジョブを投入します。
...
パッケージインストールなどインターネットアクセスが必要な操作は、スケジューラを介さずログインノードで直接実行します。
...
`[env]`セクションのmodule load等は自動適用されます。
