...
本記事は逆側の入口を取る．舞台を最小の行列多様体，すなわち円周$S^1$（$1\times1$のユニタリ行列全体$U(1)$）と$2\times2$の直交群$O(2)$・ユニタリ群$U(2)$に限定する．この舞台なら登場するものがすべて2次元平面の図に描け，更新則は[NumPy](https://numpy.org)で数行になる．道具の中心にはStiefel多様体の一般論ではなくLie代数（反Hermite行列）による乗法的更新$U\leftarrow\mathrm{e}^{-\eta S}U$を据え，Stiefel多様体との関係は最後に整理する．

前提は線形代数（固有値分解・特異値分解・随伴$\dagger$）と勾配の基礎で，多様体やLie群の予備知識は仮定しない．コードと図の生成スクリプトの全体は[リポジトリ](https://github.com/ultimatile/Zenn/tree/main/programs/intro-matrix-manifold-optimization)に置いた．
...
を選ぶ．$b$は円上にあるとは限らない与えられた複素数で，$f$の最小化は「$b$にいちばん近い円上の点を探せ」という問題である．答えは見てのとおり，$b$を原点方向に円へ射影した$z^*=b/|b|$になる．これは[以前の記事](https://zenn.dev/ultimatile/articles/unitary-procrustes)で扱った「与えられた行列に最も近いユニタリ行列を求める問題」（ユニタリProcrustes問題の特殊形）の$n=1$の場合にほかならず，$b=(b/|b|)\,|b|$は$1\times1$の[極分解](https://en.wikipedia.org/wiki/Polar_decomposition)である．答えがわかっている問題をあえて勾配法で解き，挙動を図で確かめるのが本節の趣旨である．
...
前者の「多様体の外に出た点を多様体へ戻す」操作は，一般にはレトラクションと呼ばれる道具に対応する[^retraction]．Stiefel多様体の一般論ではこちらが主役だが，本記事の主役は後者である．以下でこれを組み立てる．
...
と書ける（2つ目の等号は下のdetailsで確認する）．$\operatorname{skew}$は複素数の純虚部分$\mathrm{i}\operatorname{Im}w$を取り出す操作である．この$\operatorname{grad}f$が**Riemann勾配**，つまり多様体上での勾配の姿である．降下の一歩は，接ベクトル$-\eta\operatorname{grad}f(z)=-\eta Sz$を初速とする測地線に沿って
...
と読める．円上の乗法的な勾配降下の正体は，角度パラメータの通常の1次元勾配降下である（図2）．「パラメータ（角度）で見れば普通の勾配法，複素数・行列のままでは乗法的更新」という二重の見方は$n\times n$でも保たれる．ただし$n\geq2$では角度にあたる大域的なパラメータの便利な取り方がないため，行列のまま扱う流儀が効いてくる．

:::details 角度の微分との一致の確認

$b=r\mathrm{e}^{\mathrm{i}\varphi}$とおくと$f(\mathrm{e}^{\mathrm{i}\theta})=1+r^2-2r\cos(\theta-\varphi)$なので$f'(\theta)=2r\sin(\theta-\varphi)$．一方$\nabla f\,\bar z=2(z-b)\bar z=2(1-b\bar z)$より$S=\mathrm{i}\operatorname{Im}\bigl(2(1-b\bar z)\bigr)=-2\mathrm{i}\operatorname{Im}(b\bar z)=2\mathrm{i}\operatorname{Im}(\bar bz)=2\mathrm{i}r\sin(\theta-\varphi)=\mathrm{i}f'(\theta)$となる．

:::

コードにすると次のとおり．行列になっても構造は変わらないので，変数名は後の節と同じにしてある．
...
- Riemann勾配は$\operatorname{grad}f(U)=SU$で，Euclid勾配$\nabla f$の接空間$T_UU(n)$への直交射影に一致する（円の$\operatorname{grad}f=Sz$と同じ形）．更新則は「Riemann勾配を初速に取った測地線を長さ$\eta\|S\|$だけ進む」Riemann最急降下である[^geodesic]．
...
以降で使う道具をコードにまとめる．`np.sinc(x)`は$\sin(\pi x)/(\pi x)$なので，`np.sinc(theta / np.pi)`が$\sin\theta/\theta$を（$\theta=0$も込みで）計算する．
...
$Q^\top HQ$が対角化され，対角成分は`eigvalsh`の固有値と（並び順を除いて）一致した．図3(a)は2次形式の等高線$x^\top Hx=1$（楕円）とフレームの動きで，列ベクトル対が楕円の主軸へ向きを合わせていく．図3(b)は回転角$\theta$を横軸に取ったコストの風景で，反復は角度1変数の勾配降下として最寄りの谷$\theta^*$に落ちる．谷が$\pi/2$おきに並ぶのは，固有ベクトルの組が列の入れ替えと符号反転の分だけ等価に存在するためである．
...
なお$2\times2$対称行列には非対角成分を1回で消す回転角の閉形式があり，それを使う固有値算法が[Jacobi法](https://en.wikipedia.org/wiki/Jacobi_eigenvalue_algorithm)である（$n\times n$では$2\times2$部分問題への回転を繰り返す）．いまの勾配法は，Jacobi法が一撃で当てる回転角へ，勾配を頼りに少しずつ近づいていく方法だと見ることができる．

:::message

$O(2)$は$\det Q=+1$（回転）と$\det Q=-1$（鏡映）の2つの連結成分に分かれる．$\mathrm{e}^{-\eta S}$を掛ける更新は$\det Q$を変えないので，反復は初期値と同じ成分の中に留まる．対角化では両方の成分に大域解（固有ベクトルの一方の符号を反転したもの）があるため支障はないが，「指数写像による更新は単位元を含む成分しか歩けない」ことは覚えておく価値がある．複素の$U(n)$は連結なのでこの問題は起きない．
...
と求まる（下のdetails）．更新則はこれまでとまったく同じ形で，コードは`gd_unitary`に勾配を渡すだけである．
...
- 加法的更新＋極分解射影: $U-\eta\nabla f$と動かしてから，最も近いユニタリ行列（極因子）へ射影する．レトラクション方式の代表で，射影は特異値分解または[Newton-Schulz反復](https://zenn.dev/ultimatile/articles/unitary-procrustes-newton-schulz)で計算できる．
- 素朴な加法的更新: 射影せず$U-\eta\nabla f$のまま進む．

図4(a)のとおり，素朴な加法更新のコストは厳密解の値$f(U_{\mathrm{SVD}})$を下回ってどこまでも下がる．制約を無視すれば$UA=B$を厳密に解く$U=BA^{-1}$（一般にユニタリでない）まで行けるからで，「コストは下がっているのに答えから遠ざかる」典型例である．図4(b)のとおり多様体からの距離は$O(1)$まで育つ．一方，乗法的更新とレトラクション方式はどちらも$10^{-15}$前後に張り付き，収束の速さもこの例ではほとんど変わらない．

![cost and unitarity error for the three update rules](/images/intro-matrix-manifold-optimization/u2-procrustes.png)
*図4: ユニタリProcrustes問題での3更新則の比較（$\eta=0.15$）．(a) コスト$f(U_k)$．素朴な加法更新（赤）は制約を破り，厳密解の値$f(U_{\mathrm{SVD}})$（点線）を下回っていく．(b) 多様体からの距離$\|U_k^\dagger U_k-I\|_\mathrm{F}$．乗法的更新（青）とレトラクション方式（緑）は機械精度に張り付く．*
...
$p<n$で決定的に変わるのは群構造の有無である．縦長行列同士は掛け合わせられないので$\mathrm{St}(n,p)$は群でなく，したがってLie代数もなく，「$\Omega U$」「$\mathrm{e}^{-\eta S}U$」という本記事の書き方がそのままでは成立しない．接空間は制約の微分から

$$
T_X\mathrm{St}(n,p)=\{\Delta\in\mathbb{K}^{n\times p}\mid X^\dagger\Delta+\Delta^\dagger X=0\}
$$

と書けるが，$X$が正方でないため「反Hermite行列$\times X$」の形には畳めない（標準形は$\Delta=X\Omega+X_\perp K$で，$\Omega\in\mathfrak{u}(p)$，$K$は任意の$(n-p)\times p$行列，$X_\perp$は$X$の直交補空間の正規直交基底）．そこで，接方向へ動いた点を多様体へ戻す道具としてレトラクションが働く．よく使われるのは次の3つである．
...
- Cayley変換: $(I-\frac{\eta}{2}S)^{-1}(I+\frac{\eta}{2}S)$型の有理式で指数写像を近似する．

あるいは$\mathrm{St}(n,p)=U(n)/U(n-p)$（実なら$O(n)/O(n-p)$）という商（[等質空間](https://en.wikipedia.org/wiki/Homogeneous_space)）の見方を使えば，$U(n)$の測地線から$\mathrm{St}(n,p)$の厳密な測地線も導ける[^eas]．どの道具を選んでも，「接空間で方向を決め，多様体の上へ戻る」という骨格は図1と変わらない．
...
| 制約を保つ更新 | $\mathrm{e}^{-\eta S}U$（測地線が閉形式） | レトラクション，または商構造の測地線 |
...
- 円$S^1=U(1)$の上では，接空間・Lie代数・指数写像・Riemann勾配・レトラクションのすべてが2次元平面の図になる．$O(2)$でも自由度は角度1個で，フレームの回転として図に描ける．
- $p<n$のStiefel多様体では群構造が失われ，レトラクションまたは商構造が更新の道具になる．$p=n$のLie群はStiefel多様体の特別な場合であり，教科書がStiefel多様体から始めるのは応用が$p\ll n$に集中しているためである．
...
[^retraction]: 正確には，レトラクションは「接ベクトルから多様体上の点への滑らかな写像$R_U(\xi)$で，$R_U(0)=U$かつ$\frac{d}{dt}R_U(t\xi)\big|_{t=0}=\xi$を満たすもの」（指数写像の1次近似の族）として定義される．本文の正規化は，加法的な一歩$w=z-\eta\nabla f$に極分解射影のレトラクションを合成したものに当たる．Absil et al. の4章を参照．
...
[^eas]: [Edelman, Arias, Smith (1998)](https://arxiv.org/abs/physics/9806030)．Stiefel多様体を商$U(n)/U(n-p)$とみなした測地線や，Grassmann多様体上のNewton法・共役勾配法の公式がまとまっている．
