...
任意の$n$量子ビットユニタリ演算は1量子ビットゲートと2量子ビットゲートの列に分解できるが、実験的制約からゲート数の最小化が重要である。特に2量子ビットゲートのCNOT分解は中心的課題であり、任意の2量子ビットゲート $U \in SU(4)$ は **最大3 CNOT と15個の1量子ビットゲート** で最適に実現できることが知られている[^vw04]。
...
$$
U = e^{i\phi}\,(K_{1L} \otimes K_{1R}) \cdot \exp\!\left(i\,(c_x\,\sigma_x\sigma_x + c_y\,\sigma_y\sigma_y + c_z\,\sigma_z\sigma_z)\right) \cdot (K_{2L} \otimes K_{2R})
$$

ここで

- $K_{1L}, K_{1R}, K_{2L}, K_{2R} \in SU(2)$: 1量子ビットゲート（**局所** 演算）
- $c_x, c_y, c_z \in \mathbb{R}$: 正準（Weyl）パラメータ（**非局所** 演算を特徴づける）
- $\phi \in \mathbb{R}$: グローバル位相
- $\sigma_x\sigma_x \equiv \sigma_x \otimes \sigma_x$ 等は2量子ビットのパウリテンソル積
...
- $\mathfrak{k} = \mathrm{span}\{\sigma_\mu \otimes I, I \otimes \sigma_\mu\}_{\mu \in \{x, y, z\}}$: 局所演算の生成子（6次元）
- $\mathfrak{p} = \mathrm{span}\{\sigma_\mu \otimes \sigma_\nu\}_{\mu, \nu \in \{x, y, z\}}$: 非局所演算の生成子（9次元）

$\mathfrak{p}$ の中で互いに可換な最大部分代数（Cartan部分代数）は

$$
\mathfrak{a} = \mathrm{span}\{\sigma_x \otimes \sigma_x,\, \sigma_y \otimes \sigma_y,\, \sigma_z \otimes \sigma_z\}
$$

である。$[\sigma_x\sigma_x, \sigma_y\sigma_y] = 0$ 等は容易に確認できる。これが3次元であることが、非局所パラメータが3つであることに対応する。残りの6つの非局所生成子（$\sigma_x\sigma_y$, $\sigma_x\sigma_z$ 等）の自由度は左右の局所演算 $K_1, K_2$ に吸収される。

## Weyl chamber と正準クラスベクトル
...
$$
e^{i[(c_x + \pi/2)\sigma_x\sigma_x + c_y\sigma_y\sigma_y + c_z\sigma_z\sigma_z]} = \underbrace{i\,\sigma_x\otimes\sigma_x}_{\text{局所的}} \cdot e^{i(c_x\sigma_x\sigma_x + c_y\sigma_y\sigma_y + c_z\sigma_z\sigma_z)}
$$

のように同値類を保つ。

2. **反転**: 2成分の符号を同時に反転。例えば $(c_x, c_y, c_z) \to (-c_x, -c_y, c_z)$ は

$$
(\sigma_z \otimes I)\, e^{i\,\vec{c}\cdot\vec{\Sigma}}\, (\sigma_z \otimes I) = e^{i(-c_x, -c_y, c_z)\cdot\vec{\Sigma}}
$$

で実現される。$\vec{\Sigma} = (\sigma_x\sigma_x, \sigma_y\sigma_y, \sigma_z\sigma_z)$ とした。

3. **交換**: 2成分を入れ替え（Hadamard系のCliffordによる軸交換）。
...
これは3次元空間中の **四面体** $OA_1A_2A_3$ として視覚化できる。代表的な点は:
...
$$
|\Phi_0\rangle = \frac{|00\rangle + |11\rangle}{\sqrt{2}},\quad |\Phi_1\rangle = i\,\frac{|00\rangle - |11\rangle}{\sqrt{2}}
$$

$$
|\Phi_2\rangle = i\,\frac{|01\rangle + |10\rangle}{\sqrt{2}},\quad |\Phi_3\rangle = \frac{|01\rangle - |10\rangle}{\sqrt{2}}
$$
...
すなわち magic基底で見ると、**局所ゲートのテンソル積は実直交変換になる**。逆に、$O \in SO(4)$ ならば $\mathcal{B}\, O\, \mathcal{B}^\dagger \in SU(2) \otimes SU(2)$ である。両者は2対1準同型 $\Phi: SU(2) \times SU(2) \to SO(4)$ で結ばれており、$(A, C)$ と $(-A, -C)$ が同じ $SO(4)$ の元に写る。

### KAK分解の magic基底表現

KAK分解を magic基底に変換すると、

$$
M = \mathcal{B}^\dagger U \mathcal{B} = K_{1,m} \cdot D \cdot K_{2,m}
$$

ここで $K_{1,m}, K_{2,m} \in SO(4)$、$D = \mathrm{diag}(e^{id_0}, e^{id_1}, e^{id_2}, e^{id_3})$。
...
KAK分解を代入し、$K_{i,m} \in SO(4)$ より $K_{1,m}^T K_{1,m} = I$ かつ $D^T = D$ なので

$$
N = M^T M = K_{2,m}^T \cdot D^2 \cdot K_{2,m}
$$
...
$V = K_{2,m}$ が求まれば、$D^2$ の固有値から位相 $d_k$ が、$M = K_{1,m} D V^T$ から $K_{1,m} = M V D^{-1}$ が得られる。
...
$$
H_1 = \mathrm{Re}(N) + \mathrm{Im}(N) = \frac{(1-i)N + (1+i)N^\dagger}{2}
$$
...
magic基底で $\exp(i(c_x\sigma_x\sigma_x + c_y\sigma_y\sigma_y + c_z\sigma_z\sigma_z))$ が対角化されることから、slot encoding は
...
[^slot-fix]: 複数の文献規約が混在する領域なので、$\mathcal{B}^\dagger \sigma_x\sigma_x \mathcal{B}$ 等を自分で対角化して整合性を確認するのが安全。
...
$$
K_2 = \mathcal{B}\, V^T\, \mathcal{B}^\dagger,\qquad K_1 = \mathcal{B}\, (M V D^{-1})\, \mathcal{B}^\dagger
$$

同型 $SO(4) \cong SU(2) \otimes SU(2)$ より $K_1, K_2 \in SU(2) \otimes SU(2)$ なので、テンソル積分解 $K = A \otimes B$ ができる。実装では16要素から最大絶対値要素をピボットに取って $A, B$ を抽出する。
...
5. $c_2$ を再フォールド
6. 半端境界 tiebreak（$c_x = \pi/4$ 面で $c_z \ge 0$ を強制）

各操作で対応するCliffordを左右の2×2因子に左乗・右乗することで $U = K_1 \cdot \exp(i\,\vec{c}\cdot\vec{\Sigma}) \cdot K_2$ がグローバル位相を除いて不変に保たれる。
...
直接計算で $\mathrm{CNOT} \sim e^{i(\pi/4)\sigma_x\sigma_x}$ が示せる（局所ユニタリで $\sigma_x\sigma_z \to \sigma_x\sigma_x$ に変換）。よって正準クラスベクトルは

$$
(c_x, c_y, c_z) = \left(\frac{\pi}{4}, 0, 0\right)
$$

Weyl chamber では辺 $OA_1$ の中点 $B$ に位置する。

### SWAP

$\sigma_x\sigma_x, \sigma_y\sigma_y, \sigma_z\sigma_z$ の積関係を用いると

$$
\mathrm{SWAP} = e^{-i\pi/4}\,\exp\!\left(i\,\frac{\pi}{4}(\sigma_x\sigma_x + \sigma_y\sigma_y + \sigma_z\sigma_z)\right)
$$

正準クラスベクトルは $(\pi/4, \pi/4, \pi/4)$、Weyl chamberの頂点 $A_3$。$\det(\mathrm{SWAP}) = -1$ なのでSWAP自身は $SU(4)$ には属さないが、$e^{i\pi/4}\mathrm{SWAP} \in SU(4)$ である。

数値計算的には $N = M^T M = -i\,I$ となり全固有値が縮退する特殊ケース。実装では「$M$ が magic基底で対角」となる入力を統一的に検出する **diagonal-M bypass** を用いる[^diag-bypass]。
...
CNOTの議論から自然に $\sqrt{\mathrm{CNOT}} \sim e^{i(\pi/8)\sigma_x\sigma_x}$。正準クラスベクトルは $(\pi/8, 0, 0)$、線分 $OB$ の中点。
...
合計 **2 CNOT + 12 個の1量子ビットゲート** で実現される。

### $SU(4)$ ゲート: 3 CNOT

一般の $U \in SU(4)$ ではKAK分解と中央の正準ユニタリ $N(\alpha, \beta, \gamma) = \exp(i(\alpha\sigma_x\sigma_x + \beta\sigma_y\sigma_y + \gamma\sigma_z\sigma_z))$ の構成を組み合わせる。HadamardとCZの恒等式と隣接CNOTの打ち消しを用いて整理すると、$N(\alpha, \beta, \gamma)$ は2 CNOTで、左右の局所ゲートと合わせて合計 **3 CNOT + 15 個の1量子ビットゲート** で任意の $U \in SU(4)$ が実現される。
...
**1 CNOT の場合**: $\mathrm{SWAP} = (A_1 \otimes A_2)\,\mathrm{CNOT}\,(A_3 \otimes A_4)$ とすると、局所不変性より $\mathrm{EP}(\mathrm{SWAP}) = \mathrm{EP}(\mathrm{CNOT}) = 2/9$。しかし $\mathrm{EP}(\mathrm{SWAP}) = 0$ なので矛盾。

**2 CNOT の場合**: 各CNOTをCZに置換し、$R_z$ とCZの可換関係で整理すると、中央の非局所部分は $U = \mathrm{CZ} \cdot (R_y(a) \otimes R_y(b)) \cdot \mathrm{CZ}$ の形になる。エンタングリングパワーは
...
| chamber 内点 | 一般 $SU(4)$ | 3 |

[^impl]: 1 CNOT分岐の述語は他より厳しく $5 \times 10^{-10}$（zero軸test の $10^{-9}$ より約2倍狭い）に設定している。1 CNOT は $(\pi/4, 0, 0)$ 以外のクラスを実現できないため、内点を1 CNOT分岐に誤ルーティングすると間違ったユニタリを出力するためである。一方2 CNOT分岐への誤ルーティングではユニタリ誤差が $|c_z|$ 自身で抑えられるため、緩い tolerance を使う。
...
def kak_decompose(U):
    ...
    # 3. N = M^T M（転置）
    N = M.T @ M
    # 4. Hermitianトリック + cluster-restricted 2段eigh で V を求める
    V = simultaneous_eigenbasis(N)
    # 5. slot encoding から (cx, cy, cz)
    d = np.angle(np.diag(V.conj().T @ N @ V)) / 2
    cx = (d[0] + d[2]) / 2
    cy = (d[1] + d[2]) / 2
    cz = (d[0] + d[1]) / 2
    # ... K1, K2 復元 + テンソル積分解 + Weyl chamber 縮約 ...
...
random SU(4):       (cx, cy, cz) = ...任意の chamber 内点...      err < 2e-14
```
