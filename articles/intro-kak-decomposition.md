...
$$
U = \mathrm{e}^{\mathrm{i}\phi}\,(K_{1L} \otimes K_{1R})\exp\!\left(\mathrm{i}\,\sum_{\alpha=x,y,z}c_\alpha\,\sigma_\alpha\sigma_\alpha\right)(K_{2L} \otimes K_{2R}).
$$
...
magic基底で$\exp(i(c_x\sigma_x\sigma_x + c_y\sigma_y\sigma_y + c_z\sigma_z\sigma_z))$が対角化されることから、slot encodingは
...
数値計算的には$N = M^T M = -i\,I$となり全固有値が縮退する特殊ケース。実装では「$M$がmagic基底で対角」となる入力を統一的に検出する**diagonal-M bypass**を用いる[^diag-bypass]。
...
一般の$U \in SU(4)$ではKAK分解と中央の正準ユニタリ$N(\alpha, \beta, \gamma) = \exp(i(\alpha\sigma_x\sigma_x + \beta\sigma_y\sigma_y + \gamma\sigma_z\sigma_z))$の構成を組み合わせる。HadamardとCZの恒等式と隣接CNOTの打ち消しを用いて整理すると、$N(\alpha, \beta, \gamma)$は2 CNOTで、左右の局所ゲートと合わせて合計**3 CNOT + 15個の1量子ビットゲート**で任意の$U \in SU(4)$が実現される。
