...
本記事は[MPO-MPS積の圧縮：3手法の比較と選び方](https://zenn.dev/ultimatile/articles/tn-mpo-mps-comparison)の続きで、記号と問題設定——MMP $\hat{W}\ket{\psi}$ とボンド次元の指数爆発、ターゲット $\ket{\varphi} = \hat{W}\ket{\psi}$ をボンド次元 $D'_\text{MPS}$ のMPSに圧縮する問題——はそちらを前提とします。

ここでは3手法のうち**zip-upアルゴリズム**[^stoudenmire2010]の中身を解説します。
...
[^stoudenmire2010]: E. M. Stoudenmire and S. R. White, *Minimally Entangled Typical Thermal State Algorithms*, *New J. Phys.* **12**, 055026 (2010), [arXiv:1002.1305](https://arxiv.org/abs/1002.1305). zip-upアルゴリズムは第3節で導入されています。
...
zip-up固有の前提として、入力MPSは**右標準形(right-canonical form)**に揃えておきます。
右標準形とは、各テンソル$A^{[s]}$が
...
対称性を組み込んだMPS（[Abelian対称性テンソルの記事](https://zenn.dev/ultimatile/articles/intro-abelian-symmetric-tensor)）でもzip-upの構造はそのまま使えます。
...
同じ問題への別解である[密度行列法](https://zenn.dev/ultimatile/articles/tn-mpo-density-matrix)・[variational fitting](https://zenn.dev/ultimatile/articles/tn-mpo-variational-fitting)との精度・計算量の比較は、[入口記事](https://zenn.dev/ultimatile/articles/tn-mpo-mps-comparison)にまとめています。
