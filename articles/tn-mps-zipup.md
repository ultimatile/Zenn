...
本記事は[MPO-MPS積の圧縮：3手法の比較と選び方](https://zenn.dev/ultimatile/articles/tn-mpo-mps-comparison)の続きで、記号と問題設定——MPO-MPS積 $\hat{W}\ket{\psi}$ とボンド次元の指数爆発、ターゲット $\ket{\varphi} = \hat{W}\ket{\psi}$ をボンド次元 $D'_\text{MPS}$ のMPSに圧縮する問題——はそちらを前提とします。

ここでは3手法のうち**ジップアップ法**[^stoudenmire2010]の中身を解説します。
名前の通り「ジッパーを左から右に閉じていく」ように、サイトごとに縮約とSVD切り捨てを交互に進めることで、中間段階でも $D_\text{MPS} D_\text{MPO}$ という大きなボンドを丸ごと持たずに済む、というのが要点です。
図解は tensornetwork.org の解説ページ[^tnorg]が丁寧です。

[^stoudenmire2010]: E. M. Stoudenmire and S. R. White, *Minimally Entangled Typical Thermal State Algorithms*, *New J. Phys.* **12**, 055026 (2010), [arXiv:1002.1305](https://arxiv.org/abs/1002.1305). ジップアップ法は第3節で導入されています。

[^tnorg]: <https://tensornetwork.org/mps/algorithms/zip_up_mpo/>。図解が丁寧で実装の手がかりになります。

[^paeckel2019]: S. Paeckel, T. Köhler, A. Swoboda, S. R. Manmana, U. Schollwöck, C. Hubig, *Time-evolution methods for matrix-product states*, *Ann. Phys.* **411**, 167998 (2019), [arXiv:1901.05824](https://arxiv.org/abs/1901.05824). ジップアップ法を含む複数手法の精度と計算量の比較があります。

## 準備：右標準形（ジップアップ法の前提）

記号は入口記事に従います（$A^{[s]}_{l, \sigma_s, r}$、$W^{[s]}_{b, \sigma_s', \sigma_s, b'}$、ボンド次元 $D_\text{MPS}$・$D_\text{MPO}$、ターゲット $\ket{\varphi} = \hat{W}\ket{\psi}$）。

ジップアップ法固有の前提として、入力MPSは**右標準形(right-canonical form)**に揃えておきます。
右標準形とは、各テンソル$A^{[s]}$が
...
ジップアップ法の動機を確認します。
最も素朴なのは、ターゲット$\ket{\varphi}$のサイトテンソル

$$
B^{[s]}_{(l, b), \sigma_s', (r, b')} = \sum_{\sigma_s} W^{[s]}_{b, \sigma_s', \sigma_s, b'} A^{[s]}_{l, \sigma_s, r}
$$

（ボンド次元$D_\text{MPS} D_\text{MPO}$）を全サイト繋いでMPSを一度作り、左から右へのSVD掃引で順次$D_\text{MPS}$に切り詰める方法です。
原理的には可能ですが、実用上の難点が3つあります：
...
ジップアップ法は、これら3つを同時に回避する方法です。

## アイデア：縮約と圧縮を交互に進める

ジップアップ法の発想は単純です：「左から順に、サイトテンソル1つぶんだけ縮約して、即座にSVDで切り捨てる」。
...
全長にわたって$D_\text{MPS} D_\text{MPO}$のボンドを保持する必要がない、というのがジップアップ法の本質的な節約です。
...
このテンソルが、ジップアップ法の中で唯一$O(D_\text{MPS} D_\text{MPO})$オーダーのサイズを取る場所です。
...
最後に左右どちらかのcanonical formに変換する後処理が不要、というのはジップアップ法の地味な利点です。
...
で、$D_\text{MPO}$が大きいほどジップアップ法の優位が大きくなります。
たとえば近接相互作用Hubbard鎖の$D_\text{MPO} = 6$、$d = 4$なら比は$\sim 1/9$、より長距離・高Trotter次数の$D_\text{MPO} = 30$、$d = 2$なら$\sim 1/450$。

メモリも同様で、ジップアップ法の最大テンソルは$C^{[s]}$の$O(D_\text{MPS}^2 D_\text{MPO} d)$要素、素朴法は中間$B^{[s]}$の$O(D_\text{MPS}^2 D_\text{MPO}^2 d)$要素なので、ピークメモリで$D_\text{MPO}$ぶん節約できます。

## 精度の限界とその直感

ジップアップ法が「速いし簡単」なのは見た通りですが、結果の精度は変分的に最適ではありません。
具体的には、得られた$\ket{\tilde{\varphi}}$は、与えられたボンド次元$D'_\text{MPS}$で表されるMPSのなかで$\| \ket{\varphi} - \ket{\tilde{\varphi}} \|$を最小化するMPSとは一般に**異なります**。
...
このズレが「変分フィッティング法より精度が劣る」原因です。

実用的には：

- **誤差は単調に蓄積する**: サイトを進むにつれて累積するので、$L$が大きく$D'_\text{MPS}$の小さい場面では顕著。
- **MPOの構造に依存する**: $\hat{W}$がサイト局所に近い（$D_\text{MPO}$小）ほどジップアップ法と最適解の差は小さく、長距離・複雑な$\hat{W}$ほど差が広がる傾向。
- **後処理で改善できる**: ジップアップ法により得たMPSをそのまま使うのではなく、「同じMPSをターゲットにした変分フィッティング法の初期値」として使うと、少ないsweep回数で変分最適解に近づける。実装ではこのパターンが多用されます。

定量的には、Paeckelらのレビュー[^paeckel2019]に各手法の精度比較が載っています。それによると、ジップアップ法は時間発展で他手法より誤差が一桁程度大きくなりがちです。
精度がクリティカルなとき（虚時間冷却で最低エネルギーを精密に追う、長時間の純粋状態発展など）はジップアップ法単体ではなく、後段に変分フィッティング法を挿むのが安全です。
...
ジップアップ法の呼び出し前に、入力を必ずright-canonicalへ変換しておきます。このラッパを挟めば、呼び出し側の規約に依存せず動作が安定します。
これは$O(L D_\text{MPS}^3)$のQR掃引で、ジップアップ法本体の$O(L D_\text{MPS}^3 D_\text{MPO})$より軽いので、無条件で入れて損はありません。
...
これはジップアップ法に限った話ではないMPS全般の問題ですが、特にジップアップ法で精度を測るときには、縮退境界をまたいで打ち切らないよう$D'_\text{MPS}$を少し余裕を持って取る、または明示的な縮退検出を入れる、などの対応が有効です。
...
対称性を組み込んだMPS（[Abelian対称性テンソルの記事](https://zenn.dev/ultimatile/articles/intro-abelian-symmetric-tensor)）でもジップアップ法の構造はそのまま使えます。
ブロックスパーステンソルでのSVDは各チャージブロック独立に走るので、Step 2のSVDコストは$1/q^2$ ($q$がチャージ種数)でさらに軽くなります。
...
2. **ジップアップ法のアイデア**: 左から右へ「縮約→ SVD切り捨て→持ち越し」を繰り返し、$D_\text{MPS} D_\text{MPO}$ボンドは局所的にしか作らない。
...
6. **精度**: 変分最適ではない。誤差は累積する。精度が要るときはジップアップ法を初期値にして変分フィッティング法を後段に置く。
7. **出力**: 左標準形の新MPSが直接得られ、後処理不要。

ジップアップ法は「他の手法より速いが厳密には最適ではない」という典型的なヒューリスティックで、要求精度と計算資源のトレードオフで使い分ける道具です。
同じ問題への別解である[密度行列法](https://zenn.dev/ultimatile/articles/tn-mpo-density-matrix)・[変分フィッティング法](https://zenn.dev/ultimatile/articles/tn-mpo-variational-fitting)との精度・計算量の比較は、[入口記事](https://zenn.dev/ultimatile/articles/tn-mpo-mps-comparison)にまとめています。
