...
本記事は[MPO-MPS積の圧縮：3手法の比較と選び方](https://zenn.dev/ultimatile/articles/tn-mpo-mps-comparison)の続きで、記号と問題設定——MPO-MPS積 とボンド次元の指数爆発、ターゲット $\ket{\varphi} = \hat{W}\ket{\psi}$ をボンド次元 $D'_\text{MPS}$ のMPS $\ket{\tilde{\varphi}}$ に圧縮する問題——はそちらを前提とします。

ここでは3手法のうち**変分フィッティング法**（variational fitting、variational compressionとも呼ばれます）の中身を解説します。
ジップアップ法や密度行列法が「一掃引」で済ませるのに対し、変分フィッティング法は**「誤差最小化MPSを求める」問題そのものを最適化問題として直接解き、収束まで反復掃引する**手法です。
...
[^tnorg]: <https://tensornetwork.org/mps/algorithms/denmpo/>。密度行列法と並べて変分フィッティング法の位置づけが整理されています。

[^paeckel2019]: S. Paeckel, T. Köhler, A. Swoboda, S. R. Manmana, U. Schollwöck, C. Hubig, *Time-evolution methods for matrix-product states*, *Ann. Phys.* **411**, 167998 (2019), [arXiv:1901.05824](https://arxiv.org/abs/1901.05824). 変分フィッティング法・ジップアップ法・密度行列法の精度と計算量の比較があります。
...
変分フィッティング法では、サイト$s$に直交中心(orthogonality center)を置いた**混合標準形**を使います。
...
これがジップアップ法と共通する「太いボンドを丸ごと保持しない」という発想です。
...
ここで**混合標準形が効きます**。
直交中心をサイト$s$に置いておくと、左側は左標準形・右側も右標準形なので、$\braket{\tilde{\varphi}|\tilde{\varphi}}$は$M^{[s]}$周りで恒等に潰れ、$N = I$になります。
すると正規方程式は
...
ジップアップ法が一回掃引で終わるのに対し、こちらは収束するまで複数回掃引する点が決定的な違いです。
...
- **新MPS $\ket{\tilde{\varphi}}$の初期値**を用意します。後述するように、ここはジップアップ法や密度行列法の出力を使うのが定石です。乱数MPSでも動きますが収束が遅く、局所解に落ちやすくなります。
...
ここがジップアップ法との設計思想の違いがもっとも出る部分です。
...
[ジップアップ法](https://zenn.dev/ultimatile/articles/tn-mps-zipup)との比較が要点です。
1掃引あたりのオーダーはジップアップ法とほぼ同じ（どちらも$D_\text{MPS} D_\text{MPO}$の太いボンドを局所的にしか作らない）で、素朴法の$O(L D_\text{MPS}^3 D_\text{MPO}^3 d)$より$D_\text{MPO}$について軽いという利得もそのまま引き継ぎます。
違いは$n_\text{sweep}$という掛け算がつくこと、すなわち

$$
\frac{\text{変分フィッティング法}}{\text{ジップアップ法}} \approx O(n_\text{sweep})
$$

だけ重い、ということです。
精度を上げるためにこの定数倍を払う、というのが変分フィッティング法の立ち位置です。
...
ジップアップ法の「左から一回切るだけ」と違い、右側に残った構造の要求を**掃引の往復で何度も織り込む**ので、同じ$D'_\text{MPS}$ならジップアップ法より誤差は小さくなるのが普通です。

### 短所1：初期値依存と局所解

ALSは非凸問題に対する座標降下なので、**大域最適解が保証されません**。
特に初期値が悪いと局所解に捕まり、フィデリティが頭打ちになることがあります。
これが「ジップアップ法や密度行列法の出力を初期値にする」のが定石である理由です。
...
ジップアップ法の「一発」と違い掃引数ぶんのコストがかかるので、要求精度が緩い場面ではジップアップ法単体のほうが得なこともあります。

定量的には、Paeckelらのレビュー[^paeckel2019]に各手法の精度比較があり、十分収束させた変分フィッティング法がジップアップ法より一貫して高精度であること、ただしその代償として掃引コストがかかる点も示されています。

## 1サイト版と2サイト版

DMRGと同じく、変分フィッティング法にも1サイト(single-site)版と2サイト(two-site)版があります。
...
これがいちばん大事です。
乱数初期値からの変分フィッティング法は収束が遅く局所解にも弱いので、**ジップアップ法の出力を初期値にする**のが定石です。
「ジップアップ法で荒く速く作る → 変分フィッティング法により磨く」という二段構えにすると、少ない掃引回数で高精度に到達できます（[ジップアップ法の記事](https://zenn.dev/ultimatile/articles/tn-mps-zipup)の精度の節でも触れた通り）。
...
ジップアップ法同様、Step 1の縮約はMPOのスパース性（近接相互作用Hamiltonianなら$D_\text{MPO} \times D_\text{MPO}$のうち$O(D_\text{MPO})$しか非零でない）を活かすと実速度が大きく変わります。
...
環境テンソルもブロックスパースになり、Step 1の縮約とStep 2のQR/SVDがチャージブロックごとに独立して走るので軽くなります。
...
1. **問題**: $\hat{W}\ket{\psi}$をボンド次元$D'_\text{MPS}$のMPSで最良近似したい。ジップアップ法と同じ問題だが、「最良近似」を最適化問題として直接解くのが変分フィッティング法。
...
4. **アルゴリズム**: 環境$L_{s-1}, R_{s+1}$をキャッシュし、$M^{[s]} \leftarrow L_{s-1} A^{[s]} W^{[s]} R_{s+1}$で更新→QRにより中心を移動→環境を差分更新。収束まで往復。
5. **計算量**: 1掃引はジップアップ法とほぼ同オーダー、ただし$n_\text{sweep}$回の掃引ぶん重い。素朴法より$D_\text{MPO}$について軽いのは共通。
6. **精度**: 収束すれば与えられたボンド次元での（局所）最適解。ジップアップ法より高精度だが、初期値依存・局所解・1サイト版のボンド固定に注意。
7. **定石**: ジップアップ法（または密度行列法）の出力を初期値にして磨く二段構え。

変分フィッティング法は「掃引コストを払って精度を取りに行く」手法で、[ジップアップ法](https://zenn.dev/ultimatile/articles/tn-mps-zipup)の速さと相補的です。
精度がクリティカルな場面（虚時間冷却、長時間発展など）では、ジップアップ法や[密度行列法](https://zenn.dev/ultimatile/articles/tn-mpo-density-matrix)で初期値を作り変分フィッティング法で仕上げる、という組み合わせが実用上もっともよく使われます。
