...
ひとつめは、which-keyの`<auto>`（既定のトリガー設定）が、単キーの大文字のプレフィックスを`Z`以外は自動でトリガー登録しないという点です。which-key v3のソース（`lua/which-key/buf.lua`の`is_safe`）には、単キーのプレフィックスについて「小文字は`g`と`z`だけ、大文字は`Z`だけを安全とみなす」というガードがあります。任意の単キーを勝手に乗っ取らないための安全策です。このため`Q`はトリガーにならず、which-keyは`Q`に反応しません。これは`Q`以外の大文字単キーでも同様です。
