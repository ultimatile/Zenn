...
実数と複素数でFrobeniusノルム計算の実装が異なるため、別々の`impl`を書こうとしました。
実数の場合、Frobeniusノルム$\sqrt{\sum_i x_i^2}$を直接計算します。
複素数の場合、Frobeniusノルム$\sqrt{\sum_i |z_i|^2}$の計算に、絶対値の二乗$|z|^2 = \left(\mathrm{Re}\,z\right)^2 + \left(\mathrm{Im}\,z\right)^2$を直接計算する`Complex::norm_sqr()`を使います。
